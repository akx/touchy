...
module.exports = function TouchEmitter(emit) {
  ...
  const cb = mts.MTContactCallbackFunction.toPointer(function mtcb(
    dev,
    dataBuf,
    nFingers,
    _ts,
    ...
    for (let i = 0; i < nFingers; i++) {
      const offset = i * (0 | mts.Finger.size);
      const buf = ref.reinterpret(dataBuf, mts.Finger.size, offset);
